...
class UserService extends BaseServiceCRUD {
  ...
  async createOneUser(payload) {
    try {
      ...
      let data = await Models.User.query()
        .insert(payload)
        .returning('*').withGraphFetched('role');
      return data
    } catch (err) {
      throw err;
    }
  ...
  async getManyUser(query) {
    const builder = this.model.queryBuilder(query).join('role', 'users.roleId', 'role.id').withGraphFetched('role');
  ...
  async getMe(query, userId) {
    const builder = this.model.queryBuilder(query).findById(userId).withGraphFetched('role');
    if (this.getSearchQuery && query.q) {
      this.getSearchQuery(builder, query.q);
    }
    return builder;
  ...
  async getOneUser(id) {
    const result = await this.model.query().findById(id).withGraphFetched('role');
    if (!result) {
  ...
  async updateOneUser(id, payload) {
    const result = await this.model.query().patchAndFetchById(id, payload).withGraphFetched('role');
    if (!result) {
      throw Boom.notFound(`${this.modelName} not found`);
    }
    return result;
