...
export const actFetchCategoriesRequest = (token, offset) => {
  ...
  return (dispatch) => {
    ...
    return new Promise((resolve, reject) => {
      callApi(
        `categories?limit=${limit}&offset=${newOffset}&orderBy=-createdAt`,
        "GET",
        null,
        token
      )
...
// Get tree
export const actFetchCategoriesTreeRequest = () => {
  return (dispatch) => {
    dispatch(actShowLoading());
    return new Promise((resolve, reject) => {
      callApi(
        ...
        .then((res) => {
          if (res && res.status === 200) {
            dispatch(actFetchCategoriesTree(res.data.results));
...
export const actFetchCategoriesTree = (categories) => {
  return {
    type: Types.FETCH_CATEGORIES_TREE,
    categories,
  };
};
...
export const actDeleteCategoryRequest = (id, token) => {
  return async (dispatch) => {
    const response = await callApi(`categories/${id}`, "DELETE", null, token);
